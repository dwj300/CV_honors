...
% This example shows how to batch process a set of image files in parallel.  A
% common image processing task is to apply an image processing algorithm to a
...
%%
% If you have the Parallel Computing Toolbox software, you can use local
% workers to distribute each cycle through the loop to a different worker.
% If you also have the MATLAB(R) Distributed Computing Server(TM), you can
% run your batch processing job on a cluster.
%
% You can use workers from the default parallel profile with this command
%
%  matlabpool open 
%
